...
The file ~src/quadforms.sage~ implements Zagier's reduction theory for indefinite binary quadratic forms, and in particular uses it to compute the automorph, meaning the stabiliser in $\mathrm{SL}_2(\mathbb Z)$, and special values of partial $\zeta$-functions, attached to indefinite binary quadratic forms.
