...
mminv := Inverse(mm);
Rp := pAdicRing(p,m);
KK := CyclotomicField(3);        
chi := DirichletGroup(2*19,KK).1;
...
function IsClassicalForm(Gk,k : N:=1)
	...
	mm := Precision(Parent(Gk));
	B := Basis(ModularForms(chi^N,k),mm);
	print "Classical space is of dimension ", #B;
	// Matrix of (higher) coefficients
	C := Matrix([[Coefficient(B[i],j) : j in [1..mm-1]] : i in [1..#B]] cat [[Coefficient(Gk,j) : j in [1..mm-1]]]);
	K := Basis(Kernel(C));
	if #K eq 0 then
...
function pAdicBounds(p,m)
	...
	S  := PrecisionBound(ModularForms(2*19,2*Ks[#Ks])) + 1;
