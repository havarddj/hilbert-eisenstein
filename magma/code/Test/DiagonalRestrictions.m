...
Plan to make trivial modifications to code below so that it will compute in the quadratic case
a list of diagonal restrictions (minus constant terms) for a given input Ks of weights (exactly
as in the cubic example) ... unfinished as need to run on elastigirl
...
// Given an integer n>0, return the matrices that define tbhe Hecke operator T_n in level N.
...
function diagonal_restriction(F,k,m,As,Forms : Stabilisation := 1, pprec := m)
...
// where F is a quadratic form, whose discriminant we assume for now to be fundamental.  
function diagonal_restriction_derivative(F,p,m,As,Forms : pprec := m)

	Rp     := pAdicRing(p,pprec);
...
function Diagonal_Restriction_Derivative(F,p,m : pprec := m)
	...
	return diagonal_restriction_derivative(F,p,m,As,Forms : pprec := pprec);
...
//            E_{k,k}^{p}(1,psi_F)
// where F is a quadratic form, whose discriminant we assume for now to be fundamental.  
function WeakHarmonicMaassForm(F,p,m : pprec := m)
	...
	t0 := Cputime();
	G  := diagonal_restriction(F,1,m,As,Forms : Stabilisation := p, pprec := pprec);  // Weight k=1
	print "Time for diagonal restriction: ", Cputime() - t0;
	
	t0 := Cputime();
	Geps := diagonal_restriction_derivative(F,p,m,As,Forms : pprec := pprec);
...
//            E_{k,k}^{p}(1,psi_F) 
// where F is a quadratic form, whose discriminant we assume for now to be fundamental.
// Sept 27, 2019: ... for k in Ks.  
function Diagonal_Restriction_Ks(F,Ks,m : Stabilisation := 1)
	...
	/* print "Am:", A_m; */
	/* print ""; */
	coeff_ns := Vector([R!0: k in Ks]);
	for i := 1 to #A_p do
	    if Gcd(A_p[i],p*f) eq 1 then
		coeff_ns := coeff_ns + Vector([A_p[i]^(k-1): k in Ks]);
	    end if;
	    if Gcd(A_m[i],p*f) eq 1 then
		coeff_ns := coeff_ns + Vector([(-1)^k*(-A_m[i])^(k-1): k in Ks]);
	    end if;
	end for;
		
	Diag_Fs := Diag_Fs + 4*coeff_ns*q^n;
		
	end for;
	
	return Eltseq(Diag_Fs);
